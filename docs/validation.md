...
- **Mandate** — how strongly the spec requires it: MUST / "required" / "most important" / security rule = **3**; SHOULD / recommended = **2**; MAY / optional / Skilldex-only heuristic = **1**.
...
**Why 30 words?** Undertriggering is a known problem with Claude skills. A short description like `"Helps with debugging."` doesn't give Claude enough context to know when to invoke the skill. 30 words is a proxy for enough specificity. (This 30-word rule is a Skilldex heuristic, not a spec requirement — which is why it carries the lowest weight of the description checks.)
